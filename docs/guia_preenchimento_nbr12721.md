...
| Quadro VI (9.2.3) | Memorial descritivo dos equipamentos (instalações elétricas, hidrossanitárias, gás, incêndio, esquadrias etc.) | Memorial descritivo | Média — determinístico parcial + LLM opcional | `QUADRO6_CONFIG`; equipamentos explícitos no OCR (elevador, bomba, gás etc.); `tipo`/`acabamento` só com evidência |
| Quadro VII (9.2.4) | Acabamentos das dependências de uso privativo | Memorial descritivo | Média — determinístico parcial + LLM opcional | `QUADRO7_CONFIG`; dependência privativa + material na mesma linha OCR; `outros` quando superfície não explícita |
| Quadro VIII (9.2.5) | Acabamentos das dependências de uso comum | Memorial descritivo | Média — determinístico parcial + LLM opcional | `QUADRO8_CONFIG` |
...
- **Candidatos estruturados** (`CANDIDATOS ESTRUTURADOS QUADROS VII-VIII`) são camada operacional pós-OCR, não inferência de engenharia. Reorganizam dependência + material quando ambos aparecem na **mesma linha** (VII e VIII) ou, **somente no Quadro VII**, em **janela curta de duas linhas consecutivas** (dependência na primeira, material na seguinte, no máximo uma linha vazia entre elas). Linha só com `SACADA` ou só com `CIMENTADO` não gera candidato. Quando existir `materiais_contexto` (PISO/PAREDE/TETO), a LLM deve mapear cada material ao campo correto (`pisos`, `paredes`, `tetos`). Evidências brutas permanecem no bloco VI–VIII para contexto; candidatos orientam o patch.
- **Preenchimento determinístico do Quadro VI:** o extrator preenche `quadro6.equipamentos` a partir de candidatos estruturados quando o OCR cita equipamentos de forma explícita (ex.: `ELEVADOR01`, `BOMBA RECALQUE`, `RESERVATÓRIO`, `GÁS`). `HALL ELEVADOR` é acabamento de área comum (Quadro VIII), nunca equipamento. Campos `tipo` e `acabamento` permanecem vazios sem evidência explícita. **Validação por responsável técnico** antes de uso formal.
- **Preenchimento determinístico do Quadro VII:** o extrator preenche `quadro7.acabamentos` parcialmente a partir de candidatos `quadro7` (dependência privativa + material explícito na mesma linha OCR ou na linha imediatamente seguinte), incluindo materiais colados como `LAMINADODEMADEIRA`. Material sem superfície explícita (`PISO`/`PAREDE`/`TETO`) vai para `outros`; não infere acabamento por tipo de ambiente; janelas com múltiplas dependências ou múltiplos apartamentos são rejeitadas. Como a ordem textual de plantas não preserva a geometria, `BWC` e `BANHO` não usam janela entre linhas e exigem material explícito na mesma linha. **Validação por responsável técnico** antes de uso formal.
- **Preenchimento determinístico do Quadro VIII:** o extrator preenche `quadro8.acabamentos` automaticamente a partir de candidatos `quadro8` (dependência + material na mesma linha OCR). Material sem superfície explícita vai para `outros`; não infere piso/parede/teto. **Validação por responsável técnico** antes de uso formal.
- **Fundação geométrica (Task 28):** `documents/pdf_geometry.py` inventaria textos, linhas, retângulos e curvas com coordenadas por página, sem OCR e sem preencher planilha. A saída em `data/output/saida/geometria_pdf/` será a base para etapas futuras de detecção de ambientes, associação espacial de materiais e cálculos por vetores.
...
| Quadros VI–VIII | Médio | Conteúdo descritivo arquivado em cartório (art. 32 da Lei 4.591/64); LLM pode alucinar especificações; determinístico parcial (VI/VII/VIII) pode associar material por janela OCR sem validar plausibilidade técnica (ex.: BWC → laminado) | Enriquecimento LLM opcional; Quadros VI/VII/VIII com preenchimento determinístico parcial — **revisão técnica obrigatória** na planilha final |
