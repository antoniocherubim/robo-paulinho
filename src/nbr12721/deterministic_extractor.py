...
_RE_AREA_LAZER_COBERTA_TERREO = re.compile(
    r"[AÁ]REA\s+DE\s+LAZER\s+COBERTA.*?T[EÉ]RREO\s*=\s*([\d.,]+)\s*m",
    re.IGNORECASE,
)
_RE_AREA_LAZER_DESCOBERTA_TERREO = re.compile(
    r"[AÁ]REA\s+DE\s+LAZER\s+DESCOBERTA.*?T[EÉ]RREO\s*=\s*([\d.,]+)\s*m",
    re.IGNORECASE,
)
_RE_AREA_PAV_TERREO = re.compile(
    r"[AÁ]REA\s+PAV(?:IMENTO|\.)?\s*T[EÉ]RREO\s*:?\s*([\d.,]+)\s*m",
    re.IGNORECASE,
)
_RE_AREA_COBERTURA = re.compile(
    r"(?:COBERTURA.*?[AÁ]REA|[AÁ]REA\s+COBERTURA)\s*:?\s*([\d.,]+)\s*m",
    re.IGNORECASE,
)
_RE_PAV_TIPO_LINHA = re.compile(r"PAV\.?\s*TIPO", re.IGNORECASE)

_NOME_PAVIMENTOS_TIPO = "Pavimentos tipo"
_NOME_PAVIMENTO_TERREO = "Pavimento térreo"
_NOME_COBERTURA = "Cobertura"
...
def _extrair_qtd_pavimentos_tipo(texto: str) -> int:
    max_qtd = 0
    for linha in texto.splitlines():
        if not _RE_PAV_TIPO_LINHA.search(linha):
            continue
        for m in _RE_INTERVALO_PAV.finditer(linha):
...
def _extrair_pavimento_cobertura(texto: str) -> dict | None:
    for m in _RE_AREA_COBERTURA.finditer(texto):
